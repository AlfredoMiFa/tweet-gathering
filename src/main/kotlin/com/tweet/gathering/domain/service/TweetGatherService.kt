...
import com.tweet.gathering.infra.twitter.Twitter
...
class TweetGatherService(private val twitterAppSettings: TwitterAppSettings, private val twitterToken: TwitterToken, private val webClient: WebClient) {
    ...
    fun streamFrom(query:String):Flux<Tweet> {
        var url:String = "https://stream.twitter.com/1.1/statuses/filter.json"
        return this.webClient.mutate().baseUrl(url).build()
                .post()
                .body(BodyInserters.fromFormData("track",query))
                .header("Authorization", Twitter.buildAutHeader(twitterAppSettings, twitterToken, "POST", url, query))
